 ...
 *            class of response
 */
public abstract class BaseHTTPCommand<T extends Serializable> implements
		IAPICommand<T> {
	...
	public BaseHTTPCommand(final String url,
			final HashMap<String, String> params, RequestType type,
		...
		if (params != null) {
			...
			for (String param : params.keySet()) {
				if (sb.length() > 0)
					sb.append("&");
				sb.append(Uri.encode(param)).append("=")
						.append(Uri.encode(params.get(param)));
			}
	...
	public void run() {
		try {
			...
			httpclient.execute(setRequestData(mRequest),
					getResponseHandler(httpclient));
		} catch (Exception e) {
	...
	protected boolean cancelBecauseDeviceOffline() {
		if (mApplicationContext != null
				&& !ConnectivityUtils.isDeviceOnline(mApplicationContext)) {
	...
	protected HttpRequestBase createRequest() {
		switch (mType) {
		case GET:
			System.out.println("Data access url:" + getUrlWithParams());
	...
	protected String getUrlWithParams() {
		...
		return mUrl
				+ (mURLQueryParams != null && !mURLQueryParams.equals("") ? "?"
						+ mURLQueryParams : "");

	}

	public void responseHandlingFinished(T parsedResponse,
