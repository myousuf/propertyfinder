...
import com.test.propertyfinder.Utils.ITaskFinishedHandler.TaskResultCode;
...
public abstract class BaseTask<T extends Serializable> implements Runnable {
	 ...
	 * @param resultClazz
	 */
	public void setOnFinishedHandler(Activity activity,
			ITaskFinishedHandler<T> finishedHandler, final Class<T> resultClazz) {
		final SoftReference<Activity> activityRef = new SoftReference<Activity>(
				activity);
		...
		BroadcastReceiver finishedListener = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				final ITaskFinishedHandler<T> finishedHandler = finishedHandlerRef
						.get();

				int lowLevelErrorCode = intent.getIntExtra(
						BaseTask.BROADCAST_INTENT_EXTRA_ERROR,
						IAPICommand.ERROR_NONE);
				final int errorCode;
				final T result;
				Serializable rawResult = intent
						.getSerializableExtra(BaseTask.BROADCAST_INTENT_EXTRA_RESULT);
				if (resultClazz.isInstance(rawResult)) {
					...
					if (lowLevelErrorCode == IAPICommand.ERROR_NONE) {
						// We have no error so far, but cannot cast the result
						// data:
				...
				Runnable r = new Runnable() {
					public void run() {
						finishedHandler.onTaskFinished(mTaskCode,
